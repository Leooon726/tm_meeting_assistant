...
class PositionCalculator:
    ...
    def get_start_coords(self):
        ...
        for block_name,start_coord_idx in self.start_coord_idx_dict.items():
            start_coord_dict[block_name]=index_to_coordinate_string(start_coord_idx[0],start_coord_idx[1])
        return start_coord_dict

    def _calcuate_start_coords(self):
        for key,value in self.config.items():
            if 'coordinate' in value:
                self.start_coord_idx_dict[key]=coordinate_string_to_index(value['coordinate'])
            else:
                start_coord = self.calculate_start_coord(self.start_coord_idx_dict,key,value)
    ...
    def calculate_sizes(self):
        ...
        for block_name,constraint in self.config.items():
            ...
            if 'height' in constraint and isinstance(constraint['height'],int):
                size_dict[block_name]['height'] = constraint['height']
            else:
                subject_start_row = self.start_coord_idx_dict[block_name][1]
                constraint_object_start_row = self.start_coord_idx_dict[constraint['height']['object']][1]
                constraint_object_height = size_dict[constraint['height']['object']]['height']
                size_dict[block_name]['height'] = self.calculate_block_height(subject_start_row,constraint['height']['type'],constraint['height']['orientation'],constraint_object_start_row,constraint_object_height)
        return size_dict

    def calculate_start_coord(self,start_coord_dict,subject_name,coord_constraint):
        column_ref_object = coord_constraint['column_position']['object']
        column_ref_object_start_col_idx,_ = start_coord_dict[column_ref_object]
        column_ref_object_end_col_idx = column_ref_object_start_col_idx+self.config[column_ref_object]['width']-1
        ...
        col_idx = self.calculate_column_index(column_ref_object_start_col_idx,column_ref_object_end_col_idx,subject_width,column_constraint_type,column_constraint_orientation)
        
        row_ref_object = coord_constraint['row_position']['object']
        _,row_ref_object_start_row_idx = start_coord_dict[row_ref_object]
        row_ref_object_end_row_idx = row_ref_object_start_row_idx+self.config[row_ref_object]['height']-1
        ...
        row_idx = self.calculate_row_index(row_ref_object_start_row_idx,row_ref_object_end_row_idx,subject_height,row_constraint_type,row_constraint_orientation)

        return (col_idx,row_idx)

    @staticmethod
    def calculate_block_height(subject_start_row,constraint_type,constraint_orientation,constraint_object_start_row,constraint_object_height):
        height = None
